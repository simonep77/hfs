    ...
    internal class DirHandlerSFtp: DirHandlerBase
    {
        ...
        public DirHandlerSFtp(HfsResponseVfs resp)
        {
            ...
            this.mNormalizedPath = string.Concat(Const.URI_SEPARATOR,
                string.Concat(this.mClient.CurrenDir, this.mVfsResp.VirtualPath.Replace(this.mVfsResp.Path.Virtual, "")).MakePathRemote().TrimStart(Const.URI_SEPARATOR));
        ...
        /// <returns></returns>
        private bool isValidItem(Renci.SshNet.Sftp.ISftpFile item, bool checkFile, Regex filter)
        {
            if (checkFile)
            {
                if (item.IsDirectory)
