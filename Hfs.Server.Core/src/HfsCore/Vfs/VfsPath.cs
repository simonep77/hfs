...
namespace Hfs.Server.CODICE.VFS
{
    ...
    public class VfsPath
    {
        ...
        private UTILITY.NullableStringDictionary mParams = new UTILITY.NullableStringDictionary();
        ...
        public UTILITY.NullableStringDictionary Params
        {
        ...
        /// <returns></returns>
        public bool CheckAccess(VfsUser user, VfsAction action)
        {
            if (action == VfsAction.None)
                return true;

            VfsAccess oAccess = this.GetAccess(user);

            return (oAccess != null) ? oAccess.Check(action) : false;
        }
