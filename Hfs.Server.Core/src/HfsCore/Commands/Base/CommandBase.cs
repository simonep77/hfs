...
namespace Hfs.Server.HfsCore.Commands
{
    ...
    public abstract class CommandBase : ICommand
    {
        ...
        private void ApplyVfs()
        {
            ...
            if (this.Action.ToString()[0] != Const.VFS_ACCESS_LIST)
                HfsData.Logger.WriteMessage(ELogType.HfsAccess, $"Accesso: {this.Action,-6} - Azione: {this.ActionKey,-12} - User: {this.Response.User.Name,-12} - IP: {this.Request.IP,-15} - Risorsa: {this.Request.VPath}");
            ...
            if (!string.IsNullOrEmpty(this._request.VPathDest))
            {
                ...
                //Logga accesso dest
                if (this.ActionDest.ToString()[0] != Const.VFS_ACCESS_LIST)
                    HfsData.Logger.WriteMessage(ELogType.HfsAccess, $"Accesso: {this.ActionDest,-6} - Azione: {this.ActionKey,-12} - User: {this.Response.User.Name,-12} - IP: {this.Request.IP,-15} - Risorsa: {this.Request.VPathDest}");
        ...
        protected void SetResponseHeaders(EStatusCode code, string msg)
        {
            ...
            this.Context.Response.Headers.Add(Const.HEADER_STATUS_CODE, code.ToString("D"));
            this.Context.Response.Headers.Add(Const.HEADER_STATUS_MSG, msg);
        }
        ...
        async protected Task WriteResponseFile(IFileHandler fh, FileBlock block)
        {
            ...
            using (Stream fs = fh.OpenRead())
            {
                ...
                this.Context.Response.Headers.Add(Const.HEADER_CONTENT_LEN, lLength.ToString());
                this.Context.Response.Headers.Add(Const.HEADER_CONTENT_DISP, string.Format(Const.HEADER_CONTENT_DISP_VAL, fh.Name));
                ...
                while (lLength > 0)
                {
                    ...
                    lLength -= iRead;
                    await this.Context.Response.Body.WriteAsync(buffer, 0, iRead);
        ...
        async protected Task SaveRequestFile(IFileHandler fh, bool append)
        {
            ...
            this._bytesReceived += this.Context.Request.ContentLength.GetValueOrDefault();
            ...
            using (Stream oFs = fh.OpenWrite(!append))
            {
                ...
                try
                {
                    //Inizia lettura dati
                    int iRead = 0;
                    while ((iRead = await this.Context.Request.Body.ReadAsync(buffer, 0, Const.BUFFER_LEN)) > 0)
                    {
