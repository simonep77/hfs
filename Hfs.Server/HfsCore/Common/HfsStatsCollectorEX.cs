...
namespace Hfs.Server.CODICE.CLASSI
{
    ...
    /// </summary>
    public class HfsStatsCollectorEX: System.Web.Hosting.IRegisteredObject, IDisposable 
    {
        ...
        public HfsStatsCollectorEX()
        {
            ...
            this.mStatFile = Path.Combine(Path.GetDirectoryName(Utility.MapVirtualPath(Properties.Settings.Default.VfsFilePath)), @"hfs_stats.xml");

            //Se hosting lo registra
            
            this.mSecurityUnrestricted = AppDomain.CurrentDomain.PermissionSet.IsUnrestricted();
            ...
            try
            {
                ...
                this.mThreadWrite = new Thread(new ThreadStart(this.timerSaveStats));

                //Avvia thread scrittura differita
                this.mThreadWrite.Start();
            }
            catch (Exception e)
            {
                //Catturata
                HfsData.Logger.WriteMessage(ELogType.HfsGlobal, "Errore nell'avvio delle statistiche: {0}", e.Message);
            }
        ...
        public void Stop(bool immediate)
        {
            ...
            if (this.mThreadWrite != null && this.mThreadWrite.IsAlive)
                this.mThreadWrite.Abort();

            this.mThreadWrite = null;
