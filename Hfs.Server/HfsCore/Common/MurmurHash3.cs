...
namespace Symbiote.Core.Hashing
{
    public class MurmurHash3
    {
        ...
        private static uint Rotate(uint x, short r)
        {
            return (x << r) | (x >> (32 - r));
        }

        private static ulong Rotate(ulong x, short r)
        {
            return (x << r) | (x >> (64 - r));
        }
        ...
        public static uint Hash(byte[] key, int length, uint seed)
        {
            uint hash = seed;
        ...
        public static ulong Hash64(byte[] key, int length, uint seed)
        {
            uint part1 = seed;
            uint part2 = seed;
            ...
            {
                uint segment1 = BitConverter.ToUInt32(key, index);
                uint segment2 = BitConverter.ToUInt32(key, index + 4);
            ...
            byte[] arrP1 = BitConverter.GetBytes(part1);
            int len = arrP1.Length;
            Array.Resize(ref arrP1, len+len);
            Array.Copy(BitConverter.GetBytes(part2), 0, arrP1, len, len);

            return BitConverter.ToUInt64(arrP1, 0);
        }
    }
}