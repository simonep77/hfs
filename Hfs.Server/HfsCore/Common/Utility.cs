...
namespace Hfs.Server.CODICE.CLASSI
{
    public static class Utility
    {
        ...
        /// <summary>
        /// Dato un URI ritorna il percorso fisico del file
        /// </summary>
        /// <param name="aPath"></param>
        /// <returns></returns>
        public static string MapVirtualPath(string aPath)
        {
            HttpContext ctx = HttpContext.Current;
            try
            {
                return ctx.Server.MapPath(aPath);
            }
            catch (Exception)
            {
                //Ritorna invariato
        ...
        /// <returns></returns>
        public static string CreateReadLink(HttpContextBase context, HfsRequest req)
        {
        ...
        /// <param name="req"></param>
        public static NameValueCollection DecodeReadLink(string readlink)
        {
            try
            {
                //Decodifica
            ...
            NameValueCollection oRet = new NameValueCollection();
            oRet.Add(Const.QS_ACTION, "read");
            oRet.Add(Const.QS_USER, readlink.Substring(0, Const.HFS_LINK_PADLEN).TrimEnd(Const.HFS_LINK_PADCHAR));
            oRet.Add(Const.QS_PASS, readlink.Substring(Const.HFS_LINK_PADLEN, Const.HFS_LINK_PADLEN).TrimEnd(Const.HFS_LINK_PADCHAR));
            oRet.Add(Const.QS_VPATH, readlink.Substring((Const.HFS_LINK_PADLEN * 2) + (Const.HFS_LINK_DATEFMT.Length * 2)));
